...
const onFormHandleEmail =  async(e,formRef,navigator,setResetNumber) => {
    ...
    const config = {
        url: `${import.meta.env.VITE_BACKEND_URL}/api/auth/reset`,
        method: "POST",
        headers: {
...
const onVerifyHandle = (e,formRef,resetNumber,setResetAllowed,navigator) =>{
    ...
    if(code == resetNumber) {
        ...
        showNotification("Code is incorrent",'red');
    }
}
...
const onPasswordHandle = async(e,formRef,navigator,emailToken,setEmailToken) => {
    ...
    const config = {
        url: `${import.meta.env.VITE_BACKEND_URL}/api/auth/reset/newPassword`,
        method: "PUT",
        headers: {
            "Content-Type": "application/json",
...
export default function ResetPage({text,resetAllowed,setResetAllowed,resetNumber,setResetNumber}) {
    ...
    {if(text==="Type in your Email to reset passwort") {
        return(
            <Box
            ref={formRef}
            onSubmit={(e) => onFormHandleEmail(e,formRef,navigator,setResetNumber)}
            component="form"
            autoComplete="off"
            sx={{
                display: "flex",
            ...
            <Box
            ref={formRef}
            onSubmit = {(e) => onVerifyHandle(e,formRef,resetNumber,setResetAllowed,navigator)}
            component="form"
            autoComplete="off"
            sx={{
                display: "flex",
            ...
            return(
                <Box
                ref={formRef}
                onSubmit= {(e) => onPasswordHandle(e,formRef,navigator,emailToken,setEmailToken)}
                component="form"
                autoComplete="off"
                sx={{
                    display: "flex",
